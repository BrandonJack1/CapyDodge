        ...
        void localAxis(InputTouch touch)
        {
            if (dpadMode == EasyInputConstants.DPAD_MODE.RegisterAlways || isClicking == false)
            {
                ...
                if (Mathf.Sqrt(Mathf.Pow(touch.currentTouchPosition.x, 2) + Mathf.Pow(touch.currentTouchPosition.y, 2)) > deadZone)
                {
                    ...
                    horizontal = touch.currentTouchPosition.x * sensitivity * Time.deltaTime * 12f;
                    vertical = touch.currentTouchPosition.y * sensitivity * Time.deltaTime * 12f;
                    ...
                    {
                        character.transform.localScale = new Vector3(playerWidth * currentDirection, playerHeight, 1);
                    }
                }
                else
                {
